...
ssize_t h264_get_sps(uint8_t *src, size_t src_len, uint8_t *sps, size_t sps_len)
{
	...
	while (r < end) {
		...
		if (type == 0x7) {	/* sps */
			...
			int size = min(r1 - r, sps_len - sizeof(start_code));
			memcpy(sps + sizeof(start_code), r, size);
			return size + sizeof(start_code);
		}
...
ssize_t h264_get_pps(uint8_t *src, size_t src_len, uint8_t *pps, size_t pps_len)
{
	...
	while (r < end) {
		...
		int type = r[0] & 0x1f;
		if (type == 0x8) {	/* pps */
			memcpy(pps, start_code, sizeof(start_code));

			int size = min(r1 - r, pps_len - sizeof(start_code));
			memcpy(pps + sizeof(start_code), r, size);
			return size + sizeof(start_code);
		}
